...
# One job by sample
# steps : Configuration file -> slurm script -> job submission
...
# Input sample name
batch="LN_KO_BIT LN_WT_IG LN_WT_BIT T_KO_IG T_KO_BIT T_WT_IG T_WT_BIT"

# Input
path_ref_gex=/home/dutel/data/reference/other/gex/refdata-gex-GRCm39-2024-A
path_ref_vdj=/home/dutel/data/reference/other/vdj/refdata-cellranger-vdj-GRCm38-alts-ensembl-7.0.0
path_ref_feature=/home/dutel/data/reference/other/antibody_capture/feature.csv
...
# Output & Log
path_libraries=/home/dutel/script/cellranger_multi/configuration # configuration file .csv folder
path_script=/home/dutel/script/cellranger_multi/slurm_job # where to save slurm job scripts
output=/labos/UGM/Recherche/Bi-spe/output_cellranger_multi_alignment # cellranger multi output folder 
...
for b in ${batch};do
    ...
    echo -e "\n[samples]" >> $conf2
    echo "sample_id,hashtag_ids" >> $conf2
...
for b in ${batch};do
    ...
    conf2=${path_libraries}/config.${b}.csv
    # Create slurm script 
    script=/home/dutel/script/cellranger_multi/slurm_job/cr.multi_${b}_${today}.sh
    ...
    \n#SBATCH --mem=40gb \
    \n#SBATCH --error=/home/dutel/log/$today.cr_multi.${b}.%j.err \
    \n#SBATCH --output=/home/dutel/log/$today.cr_multi.${b}.%j.out\n" >> $script
